...
```sh
npx --yes @shendeguize/agent-organon@1.0.0-rc.1 check --product advised --agent codex --scope project --project /path/to/workspace
```
...
```sh
npx --yes @shendeguize/agent-organon@1.0.0-rc.1 init --product advised --agent codex --scope project --project /path/to/workspace --domain SoftwareEngineering --philosophy SoftwareEngineering
```
...
> Use `organon-software-engineering` with this project’s explicitly selected `PHILOSOPHY.md` as the adopted baseline. Assess the material’s grounds, applicability and possible conflicts. Return a read-only report; do not edit files or adopt a proposal.
...
```sh
node /path/to/extracted/package/installer/organon.mjs install --product advised --agent codex --scope project --project /path/to/workspace --from /path/to/shendeguize-advised-organons-1.0.0-rc.1.tgz --domain SoftwareEngineering
```
...
[Understand the concepts and boundaries](https://shendeguize.github.io/AdvisedOrganons/understand) · [GitHub Releases](https://github.com/shendeguize/AdvisedOrganons/releases)
